...
<template>
  <div class="mx-auto max-w-4xl px-6 py-12" style="background: var(--color-background);">
    ...
    <p class="mt-2 opacity-75" style="color: var(--color-text-dark);">
      Past and current plant orders.
    </p>
    <div v-if="loading" class="mt-14 text-center opacity-75" style="color: var(--color-text-dark);">Loading…</div>
    <div
      v-else-if="orders.length === 0"
      class="card mt-12 rounded-2xl border border-[var(--color-border)] p-12 text-center"
      style="color: var(--color-text); opacity: 0.9;"
    >
      No orders yet.
      <router-link to="/plants" class="font-medium hover:underline" style="color: var(--color-forest);">Browse plants</router-link>
    </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <template v-if="order.status === 'delivered'">
              ...
              <button
                ...
                class="rounded-lg p-2 transition-opacity hover:opacity-90 disabled:opacity-50"
                style="color: #b91c1c;"
                aria-label="Remove from my orders"
                :disabled="deleting === order.id"
                @click="openDeleteModal(order.id)"
              >
                <Trash2 class="h-4 w-4" stroke-width="1.5" />
    ...
    <ConfirmModal
      ...
      :title="orderToDelete && orders.find(o => o.id === orderToDelete)?.status === 'delivered' ? 'Remove Order' : 'Delete Order'"
      :message="orderToDelete && orders.find(o => o.id === orderToDelete)?.status === 'delivered' ? 'Remove this order from your list? You can still download the receipt anytime. Admin will still see this order.' : 'Are you sure you want to delete this order? This action cannot be undone.'"
      confirm-text="Remove"
      cancel-text="Cancel"
      danger
